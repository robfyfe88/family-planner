...
export default function ActivitiesPlanner() {
  const [members, setMembers] = useLocalStorageState<Member[]>(
    `${STORE_KEY}:members`,
    [{ id: uid(), role: "parent", name: "Parent 1", shortLabel: "P1", color: palette[0] }]
  );
  const [showParent2, setShowParent2] = useLocalStorageState<boolean>(`${STORE_KEY}:showParent2`, false);

  useEffect(() => {
    if (showParent2 && !members.some(m => m.role === "parent" && m.shortLabel === "P2")) {
      setMembers(prev => [...prev, { id: uid(), role: "parent", name: "Parent 2", shortLabel: "P2", color: palette[1] }]);
    }
    if (!showParent2) {
      setMembers(prev => prev.filter(m => !(m.role === "parent" && m.shortLabel === "P2")));
    }
    ...
    <div className="space-y-6">
      ...
      <section className="card space-y-4">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {members.filter(m => m.role === "parent" && m.shortLabel === "P1").map((m) => (
            <MemberCard key={m.id} member={m} onChange={(patch) => updateMember(m.id, patch)} onRemove={undefined} />
          ))}
          {showParent2 ? (
            members.filter(m => m.role === "parent" && m.shortLabel === "P2").map((m) => (
              <div key={m.id} className="space-y-2">
                <MemberCard member={m} onChange={(patch) => updateMember(m.id, patch)} onRemove={() => setShowParent2(false)} />
                <div>
                  <Button variant="outline" className="text-red-600" onClick={() => setShowParent2(false)}>
                    Remove Parent 2
                  </Button>
                </div>
              </div>
    ...
    <div className="p-3 border rounded-2xl">
      <div className="flex flex-wrap items-end gap-3 justify-between">
        <div className="inline-flex items-center gap-2">
          ...
          <Input className="w-56" value={member.name} onChange={(e) => onChange({ name: e.target.value })} placeholder="Name" />
        </div>
        <div className="flex items-end gap-3">
          <div className="flex flex-col gap-1">
            <Label className="text-sm">Label</Label>
            <Input className="w-24" value={member.shortLabel} onChange={(e) => onChange({ shortLabel: e.target.value.slice(0, 2).toUpperCase() })} />
          </div>
          <div className="flex flex-col gap-1">
