...
function buildMonthMatrix(monthAnchor: Date) {
    ...
    const gridStart = addDays(first, -first.getDay()); // Sunday grid
    const cells: Date[] = [];
    for (let i = 0; i < 42; i++) cells.push(addDays(gridStart, i));
    return { cells };
}

/** ───────────── LocalStorage helper (prevents clobber) ───────────── */
...
export default function AnnualLeavePlanner() {
    // Persisted states (no restore/persist effects anymore)
    ...
    // Persisted anchor (as ISO) with a Date wrapper
    ...
    // IMPORTANT FIX: applying an override ensures the date is a closure (so it shows immediately & persists)
    ...
    const applyPlan = () => {
        ...
        setAppliedPlan(res.plan); // persisted
    };
    const clearPlan = () => setAppliedPlan(null);

    /** Auto-reapply on any input change (once plan exists) */
    useEffect(() => {
