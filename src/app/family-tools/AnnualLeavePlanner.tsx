...
export default function AnnualLeavePlanner({ initial }: { initial: AnnualData }) {
  ...
  const dragStartXY = React.useRef<{ x: number; y: number } | null>(null);
  ...
  const onPointerMoveCell = (e: React.PointerEvent) => {
    if (!dragging || !dragStartXY.current) return;
    const dx = Math.abs(e.clientX - dragStartXY.current.x);
    const dy = Math.abs(e.clientY - dragStartXY.current.y);
    if (!dragMoved && (dx > DRAG_THRESHOLD_PX || dy > DRAG_THRESHOLD_PX)) {
      setDragMoved(true);
    }
  };
  ...
  const commitDrag = () => {
    ...
    // ignore clicks that didn’t move
    if (!dragMoved) {
      setDragging(false);
      setDragStart(null);
      setDragEnd(null);
  ...
  return (
    <div className="space-y-6">
      {/* Header: hide the quick add button for caregivers */}
      ...
      {/* Parents + Controls — completely hidden for caregivers */}
      {!isCaregiver && (
      ...
      {/* Stats — always shown (allocations) */}
      ...
      <section className="card">
        ...
        <div className="hidden md:block">
          ...
          <div className="grid grid-cols-7 gap-px bg-[var(--border-color)] p-1 overflow-hidden select-none touch-none">
            {cells.map((d) => {
              const id = ymd(d);
              const isBH = bankHolidaySet.has(id);
              ...
              return (
                <div
                  ...
                  onPointerMove={onPointerMoveCell}
                  onPointerEnter={() => onPointerEnterCell(id)}
                  onPointerUp={(e) => { e.preventDefault(); commitDrag(); }}
                  onPointerCancel={() => { setDragging(false); setDragStart(null); setDragEnd(null); setDragMoved(false); dragStartXY.current = null; }}
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button
                          ...
                          onPointerDown={(e) => { e.stopPropagation(); setDragging(false); setDragMoved(false); dragStartXY.current = null; }}
                      ...
                      <DropdownMenuContent align="end" className="w-64">
                        ...
                        {eventsToday.length > 0 && (
                          <>
                            ...
                            {eventsToday.map((ev) => (
                              <DropdownMenuItem key={ev.id} onClick={() => openEditDialogFor(ev)}>
                        ...
                        <DropdownMenuSeparator />
                        {!isCaregiver && (
                          <DropdownMenuItem onClick={() => openAddDialogFor(id)}>
      ...
      {/* Add/Edit Holiday Event (Dialog) — accessible only if opened (we hid quick-add for caregivers) */}
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editEventId ? "Edit holiday event" : "Add holiday event"}</DialogTitle>
            <DialogDescription>
              {editEventId ? "Update the details for this event." : "Quickly add an all-day holiday/event to your household calendar."}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="flex flex-col gap-1">
                  <Label>Title</Label>
                  <Input value={eventDraft.title} onChange={(e) => setEventDraft({ ...eventDraft, title: e.target.value })} placeholder="e.g. Grandparents visit" />
                </div>
                <div className="flex flex-col gap-1">
                  <Label>Color</Label>
                  <Input type="color" value={eventDraft.color ?? "#c084fc"} onChange={(e) => setEventDraft({ ...eventDraft, color: e.target.value })} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="flex flex-col gap-1">
                  <Label>Start</Label>
                  <Input type="date" value={eventDraft.startDateISO} onChange={(e) => setEventDraft({ ...eventDraft, startDateISO: e.target.value })} />
                </div>
                <div className="flex flex-col gap-1">
                  <Label>End</Label>
                  <Input type="date" value={eventDraft.endDateISO} onChange={(e) => setEventDraft({ ...eventDraft, endDateISO: e.target.value })} />
                </div>
              </div>

              <div className="flex items-center gap-2">
                <Checkbox id="allDay" checked={!!eventDraft.allDay} onCheckedChange={(v) => setEventDraft({ ...eventDraft, allDay: !!v })} />
                <Label htmlFor="allDay">All day</Label>
              </div>

              <div className="flex flex-col gap-1">
                <Label>Notes</Label>
                <Textarea rows={3} value={eventDraft.notes ?? ""} onChange={(e) => setEventDraft({ ...eventDraft, notes: e.target.value })} placeholder="Optional details…" />
              </div>
            </div>
          )}

          <DialogFooter className="mt-4 w-full flex items-center justify-between">
            {editEventId ? (
              <Button
                variant="default"
                  ...
                  start(async () => {
                    if (editEventId) {
                      ...
                      setHolidayEvents((prev) =>
                        prev.map((e) => (e.id === updated.id ? updated : e)).sort((a, b) => (a.startDate < b.startDate ? -1 : 1))
                      );
                    } else {
                      const created = await createHolidayEvent(payload);
                      setHolidayEvents((prev) =>
                        [...prev, created].sort((a, b) => (a.startDate < b.startDate ? -1 : 1))
                      );
                    }
                    setAddOpen(false);
                    setEditEventId(null);
...
function EventPill({ title, color, onClick }: { title: string; color: string | null; onClick?: (e: React.MouseEvent) => void; }) {
  return (
    <button type="button" onClick={onClick} className="inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded hover:bg-gray-100" title={title}>
...
function MobileMonthList({
    ...
    return (
      ...
      >
        ...
        <div className="flex-1">
          <div className="flex flex-wrap gap-1">{chips}</div>
          <div className="mt-1 flex flex-wrap gap-2 text-[11px] opacity-80 items-center">
  ...
  return (
    <div className="space-y-2">
      <div className="text-xs opacity-70 -mt-1 mb-1">Tap to set closures • use ⚙ for overrides • ＋ for events</div>
