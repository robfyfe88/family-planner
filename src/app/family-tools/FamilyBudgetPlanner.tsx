...
import {
    ...
    fetchPotPlans,
    upsertPotPlan,
} from "@/app/app/budget/pots-actions";

type Owner = "joint" | "A" | "B";
type RecurrenceUI = "recurring" | "oneoff";
type Scope = "this-month" | "from-now-on" | "entire-range";
...
function migrateBudgetState(parsed: Partial<BudgetState>): BudgetState {
    ...
    const incomes: Row[] = Array.isArray(parsed.incomes)
        ? parsed.incomes.map((r) => ({ ...r, owner: (r.owner ?? "joint") as Owner, recurrence: (r.recurrence ?? "recurring") as RecurrenceUI }))
        : [];
    const expenses: Row[] = Array.isArray(parsed.expenses)
        ? parsed.expenses.map((r) => ({ ...r, owner: (r.owner ?? "joint") as Owner, recurrence: (r.recurrence ?? "recurring") as RecurrenceUI }))
        : [];
...
function Donut({
    ...
    return (
        <div className="relative inline-flex items-center justify-center mb-4" style={{ width: size, height: size }}>
...
export default function FamilyBudgetPlanner() {
    ...
    const thisMonth = currentMonthIndex + 1;
    React.useEffect(() => {
        let cancelled = false;
        ...
        (async () => {
            try {
                const [rows, pots, plans] = await Promise.all([
                    fetchBudgetRowsForMonth(currentYear, summaryMonthIdx + 1),
                    fetchPots(),
                    fetchPotPlans(currentYear),
                ]);
                if (cancelled) return;

                const withRecurrence = (r: any): Row => ({
                    id: r.id as string,
                    label: r.label,
                    amount: r.amount,
                ...
                setState((s) => ({
                    ...s,
                    incomes: rows.incomes.filter((r: any) => typeof r.id === "string").map(withRecurrence),
                    expenses: rows.expenses.filter((r: any) => typeof r.id === "string").map(withRecurrence),
                }));
                ...
                for (const p of pots as any[]) {
                    ...
                    if (!hasAny) {
                        ...
                        if (seed !== 0) {
                            savingsYear[new Date().getMonth()].values[p.id] = seed;
                        }
                    }
                }
    ...
    }, [setState, summaryMonthIdx, currentYear]);
    ...
    const commitIncomeLabel = (id: string, rawLabel: string) => {
        ...
        const curr = state.incomes.find((r) => r.id === id) || { id, label: finalLabel, amount: 0, owner: "joint" as Owner, recurrence: "recurring" as RecurrenceUI };
        const scopeToUse: Scope = (curr.recurrence === "oneoff") ? "this-month" : scope;

        (async () => {
            const saved = await upsertBudgetRowScoped("income", {
                id: id.startsWith("tmp_") ? undefined : id,
                label: finalLabel,
                amount: curr.amount ?? 0,
                owner: curr.owner ?? "joint",
                year: new Date().getFullYear(),
                month1to12: summaryMonthIdx + 1,
                scope: scopeToUse,
            });
            if (saved?.id) {
                setState((s) => ({
    ...
    const setIncomeAmount = (id: string, amount: number) => {
        ...
        if (!label) return;
        const scopeToUse: Scope = (row?.recurrence === "oneoff") ? "this-month" : scope;
    ...
    const addIncome = () => {
        ...
        setState((st) => ({
            ...st,
            incomes: [...st.incomes, { id, label: "", amount: 0, owner: st.mode === "split" ? "A" : "joint", recurrence: "recurring" }],
        }));
    };

    const rmIncome = async (id: string, scope: Scope = "from-now-on") => {
        setState((st) => ({ ...st, incomes: st.incomes.filter((r) => r.id !== id) }));
        if (!id.startsWith("tmp_")) {
            try {
                await deleteBudgetRowScoped(id, scope, currentYear, thisMonth);
            } catch { }
        }
    };
    ...
    const commitExpenseLabel = (id: string, rawLabel: string) => {
        const finalLabel = normalizeLabel(rawLabel);
        if (!finalLabel) return;
        const curr = state.expenses.find((r) => r.id === id) || { id, label: finalLabel, amount: 0, owner: "joint" as Owner, recurrence: "recurring" as RecurrenceUI };
        const scopeToUse: Scope = (curr.recurrence === "oneoff") ? "this-month" : scope;
    ...
    const addExpense = () => {
        ...
        setState((st) => ({
            ...st,
            expenses: [...st.expenses, { id, label: "", amount: 0, owner: st.mode === "split" ? "A" : "joint", recurrence: "recurring" }],
        }));
    };

    const rmExpense = async (id: string, scope: Scope = "from-now-on") => {
    ...
    const exportCSV = () => {
        const lines: string[] = [];
        lines.push("Section,Owner,Label,Recurrence,Amount");
        const ownerLabel = (o?: Owner) => (o ?? "joint");
        const recLabel = (r?: RecurrenceUI) => r === "oneoff" ? "One-off" : "Recurring";
        for (const r of state.incomes) lines.push(`Income,${ownerLabel(r.owner)},"${csv(r.label)}",${recLabel(r.recurrence)},${safe(r.amount)}`);
        for (const r of state.expenses) lines.push(`Expense,${ownerLabel(r.owner)},"${csv(r.label)}",${recLabel(r.recurrence)},${safe(r.amount)}`);
        lines.push("");
    ...
    const donutSize = isSmUp ? 200 : 160;
        ...
        <div className="space-y-4 sm:space-y-6">
            <section className="card">
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 sm:gap-6 items-center">
                    <div className="lg:col-span-2">
                        <div className="flex flex-wrap flex-col items-start gap-2 justify-between">
                    ...
                    <div className="lg:col-span-2 w-full mt-22">
            ...
            <section className="card">
                <BudgetEditorCompact
                    ...
                    onIncomeAmountCommit={setIncomeAmount}
                    onExpenseAmountCommit={setExpenseAmount}
    ...
    return (
        <Input
            ...
            onBlur={() => {
                ...
                onCommit(Number.isFinite(n) ? round2(n) : 0);
            }}
            onKeyDown={(e) => {
                if (e.key === "Enter") (e.target as HTMLInputElement).blur();
            }}
...
function BudgetEditorCompact({
    ...
    onIncomeAmountCommit,
    onExpenseAmountCommit,
    onAddIncome,
    onAddExpense,
    onRemoveIncome,
    onRemoveExpense,
    ...
    onIncomeChange: (id: string, patch: Partial<Row>) => void;
    onExpenseChange: (id: string, patch: Partial<Row>) => void;

    onIncomeLabelCommit: (id: string, label: string) => void;
    onExpenseLabelCommit: (id: string, label: string) => void;
    onIncomeAmountCommit: (id: string, amount: number) => void;
    onExpenseAmountCommit: (id: string, amount: number) => void;
    ...
    const onAmountCommit = tab === "income" ? onIncomeAmountCommit : onExpenseAmountCommit;
            ...
            <div className="flex flex-wrap items-center gap-3">
                ...
                {rows.length > 0 && (
                    <div className="flex items-center gap-2">
                        ...
                        <Select value={ownerFilter as string} onValueChange={(v) => setOwnerFilter(v as any)}>
            ...
            <div className="sm:hidden space-y-2">
                {filtered.map((r, idx) => (
                    <div key={r.id} className="border rounded-xl p-3">
                        ...
                        {mode === "split" && (
                            <div className="mt-2">
                                <OwnerBadge owner={r.owner ?? "joint"} />
                            </div>
                        )}
                    </div>
                ))}
                ...
                <div className="max-h-80 overflow-auto">
                    <table className="w-full text-sm">
                        <thead className="sticky top-0 bg-white">
                            <tr>
                                <th className="text-left px-3 py-2 w-[34%]">Label</th>
                                <th className="text-left px-3 py-2 w-[18%]">Recurrence</th>
                                {mode === "split" && <th className="text-left px-3 py-2 w-[18%]">Owner</th>}
                        ...
                        <tbody className="[&>tr:nth-child(even)]:bg-[rgba(0,0,0,0.02)]">
                                ...
                                <tr key={r.id} className="group">
                                    ...
                                    <td className="px-3 py-2 text-right">
                                        <MoneyCell
                                            ...
                                            onCommit={(v) => onAmountCommit(r.id, v)}
                                        />
                                    </td>
                            ...
                            {filtered.length === 0 && (
                                <tr>
                                    <td colSpan={mode === "split" ? 5 : 4} className="px-3 py-6 text-center text-sm opacity-70">
